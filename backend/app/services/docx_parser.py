...
class VesselData(BaseModel):
    ...
    cap_tau: str = "KhÃ´ng xÃ¡c Ä‘á»‹nh"
    ho_ten: str = ""
    dia_chi: str = ""
    may_chinh: float = 0.0
    han_dk: str = ""
...
def parse_docx(docx_file: str | BinaryIO) -> ParsedVesselData:
    ...
    return ParsedVesselData(
        ...
        province_name=fields.get("province_name") or extract_province_code(registration_number),
        lmax=lmax,
        power_kw=power_kw,
        ...
        fishing_gear=fields.get("fishing_gear") or "KhÃ´ng xÃ¡c Ä‘á»‹nh",
    )


def parse_vessel_docx(file_path: str) -> VesselData:
    """Backward-compatible wrapper for the batch processor."""
...
def extract_province_code(registration_number: str) -> str:
    ...
    match = re.match(r"\s*([A-Za-zÄÄ‘]{1,4})\s*-", registration_number)
    if not match:
...
def _extract_fields(text: str) -> dict[str, str]:
    ...
    certificate_code = re.search(r"Sá»‘[^\n]*?:\s*[\d.]+/([A-ZÄ]{2})", text, re.I)
    if certificate_code:
...
def _parse_inspection_code(value: str) -> InspectionTypeEnum | None:
    normalized = _normalize(value).replace("d", "d")
    normalized = normalized.upper().replace("Ä", "D")
    return _INSPECTION_CODES.get(normalized)


def _parse_inspection_type(value: str) -> InspectionTypeEnum:
    code_match = re.fullmatch(r"\s*([A-ZÄ]{2})\s*", value, re.I)
    if code_match:
...
def _parse_date(value: str | None, default: date) -> date:
    ...
    if not match:
        match = re.search(r"ngÃ y\s+(\d{1,2})\s+thÃ¡ng\s+(\d{1,2})\s+nÄƒm\s+(\d{4})", value, re.I)
    if not match:
        return default
