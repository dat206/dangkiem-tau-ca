...
export function VesselsScreen({ density, role }) {
  ...
  const [lmaxGrp, setLmaxGrp] = useState("all");
      ...
      <Card pad={16} style={{marginBottom:16}}>
        <div style={{display:"grid", gridTemplateColumns:"1.4fr 1fr 1fr 1fr auto", gap:10, alignItems:"end"}}>
          ...
          <div className="form-row">
            ...
            <select className="input select" value={lmaxGrp} onChange={e=>setLmaxGrp(e.target.value)}>
              <option value="all">Tất cả</option>
            ...
            <div style={{padding:"20px 24px", borderBottom:"1px solid var(--border-soft)", display:"flex", justifyContent:"space-between", alignItems:"flex-start"}}>
              <div>
                ...
                <div style={{display:"flex", gap:6, marginTop:8}}>
                  <Badge tone={inspTone(drawer.insp)}>{drawer.insp} · {inspLabel(drawer.insp)}</Badge>
                  <Badge tone={hullTone(drawer.hull)}>{drawer.hull}</Badge>
                  <Badge tone="slate">{lmaxGroup(drawer.lmax)}</Badge>
                </div>
              </div>
...
// ─── REPORTS / GENERATE ──────────────────────────────────────────────────────
export function ReportsGenerateScreen() {
  const [quarter, setQuarter] = useState("II");
  const [year, setYear] = useState(2026);
  const [selected, setSelected] = useState(["QN","HP","TH"]);
  const [fmt1, setFmt1] = useState(true);
  const [fmt2, setFmt2] = useState(true);
  const [phase, setPhase] = useState("ready"); // ready | loading | done
...
// ─── REPORTS / HISTORY ───────────────────────────────────────────────────────
export function ReportsHistoryScreen({ density, role }) {
