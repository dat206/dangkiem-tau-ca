// data.jsx — mock data, palette helpers, formatters
// All names below are clearly demo data for a fisheries vessel inspection app.

const PROVINCES = [
  { code: "QN",    name: "QN (Tỉnh Quảng Ninh)",           count: 84 },
  { code: "HP",    name: "HP (Thành phố Hải Phòng)",        count: 51 },
  { code: "TB",    name: "TB (Tỉnh Thái Bình)",             count: 19 },
  { code: "ND",    name: "ND (Tỉnh Nam Định)",              count: 22 },
  { code: "NB",    name: "NB (Tỉnh Ninh Bình)",             count: 15 },
  { code: "TH",    name: "TH (Tỉnh Thanh Hóa)",             count: 31 },
  { code: "NA",    name: "NA (Tỉnh Nghệ An)",               count: 28 },
  { code: "HT",    name: "HT (Tỉnh Hà Tĩnh)",               count: 12 },
  { code: "QB",    name: "QB (Tỉnh Quảng Bình)",            count: 18 },
  { code: "QT",    name: "QT (Tỉnh Quảng Trị)",             count: 9  },
  { code: "TTH",   name: "TTH (Tỉnh Thừa Thiên Huế)",       count: 25 },
  { code: "ĐN",    name: "ĐN (Thành phố Đà Nẵng)",          count: 24 },
  { code: "QNg",   name: "QNg (Tỉnh Quảng Ngãi)",           count: 33 },
  { code: "BĐ",    name: "BĐ (Tỉnh Bình Định)",             count: 41 },
  { code: "PY",    name: "PY (Tỉnh Phú Yên)",               count: 20 },
  { code: "KH",    name: "KH (Tỉnh Khánh Hòa)",             count: 47 },
  { code: "NT",    name: "NT (Tỉnh Ninh Thuận)",            count: 14 },
  { code: "BT",    name: "BT (Tỉnh Bình Thuận)",            count: 39 },
  { code: "BR-VT", name: "BR-VT (Tỉnh Bà Rịa - Vũng Tàu)",   count: 42 },
  { code: "HCM",   name: "HCM (Thành phố Hồ Chí Minh)",       count: 35 },
  { code: "TG",    name: "TG (Tỉnh Tiền Giang)",            count: 18 },
  { code: "BTr",   name: "BTr (Tỉnh Bến Tre)",              count: 22 },
  { code: "TV",    name: "TV (Tỉnh Trà Vinh)",              count: 16 },
  { code: "ST",    name: "ST (Tỉnh Sóc Trăng)",             count: 21 },
  { code: "BL",    name: "BL (Tỉnh Bạc Liêu)",              count: 15 },
  { code: "CM",    name: "CM (Tỉnh Cà Mau)",                count: 38 },
  { code: "KG",    name: "KG (Tỉnh Kiên Giang)",            count: 55 },
  { code: "HG",    name: "HG (Tỉnh Hậu Giang)",             count: 10 },
];

// 28 vessels — varied across all inspection types, materials, Lmax groups, provinces.
// Names are obviously demo (common Vietnamese given names, no real fishermen).
const VESSELS = [
...
// Recent uploads — mix of statuses
...
// Province totals (top 6 for bar chart)
const PROVINCE_TOTALS = PROVINCES.slice().sort((a,b)=>b.count-a.count).slice(0,6);

// Inspection-type donut data
const INSP_TOTALS = [
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function provName(code){ const p = PROVINCES.find(x=>x.code===code); return p ? p.name : code; }
function inspLabel(k){ return ({HN:"Hàng năm",TĐ:"Trên đà",ĐK:"Định kỳ",GS:"Giám sát"})[k] || k; }
function lmaxGroup(v){
...
// Sidebar nav items — emoji and svg variants
const NAV_MAIN = [
