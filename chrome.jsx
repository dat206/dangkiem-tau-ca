// chrome.jsx — Sidebar, Header, shared UI primitives (Badge, Card, IconBtn, etc.)
// Components are exported on window so other Babel scripts can use them.
...
// ─── Brand mark — minimal wave + anchor ──────────────────────────────────────
...
// ─── Wave / nautical chart background (subtle SVG pattern) ───────────────────
const WaveBg = ({ opacity = 0.06 }) => (
...
// ─── Badge component ─────────────────────────────────────────────────────────
const Badge = ({ tone = "blue", soft = true, children, dot = false, size = "md" }) => {
  const tones = {
...
const inspTone   = (k) => ({ HN:"blue", TĐ:"purple", ĐK:"green", GS:"amber" })[k] || "gray";
const hullTone   = (m) => ({ "Gỗ":"brown", "Thép":"slate", "FRP":"teal" })[m] || "gray";

// ─── Card ────────────────────────────────────────────────────────────────────
const Card = ({ children, style, pad = 20, ...rest }) => (
  <div style={{
...
// ─── Button ──────────────────────────────────────────────────────────────────
const Button = ({ variant = "primary", icon, size = "md", children, style, disabled, ...rest }) => {
...
// ─── IconBtn ─────────────────────────────────────────────────────────────────
const IconBtn = ({ icon, title, tone = "slate", size = 30, onClick }) => {
...
// ─── Sidebar ─────────────────────────────────────────────────────────────────
const Sidebar = ({ active, onNav, role, iconStyle, currentUser }) => {
  const { NAV_MAIN, NAV_ADMIN } = window.AppData;
  const renderItem = (n) => {
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="side-user">
        ...
        <button title="Đăng xuất" onClick={()=>onNav("login")} style={{
...
// ─── Header ──────────────────────────────────────────────────────────────────
const Header = ({ title, subtitle, breadcrumb, actions, lastUpdated }) => (
...
// ─── Empty state ─────────────────────────────────────────────────────────────
const EmptyState = ({ icon = "folder", title, hint, action }) => (
...
Object.assign(window, {
  Icon, NAV_ICON, BrandMark, WaveBg, Badge, inspTone, hullTone,
  Card, Button, IconBtn, Sidebar, Header, EmptyState,
});
